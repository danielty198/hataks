...
export default function MichlolPage() {
  ...
  const [user] = useUser();
  const LAST_TEMPLATE_KEY = "lastTemplateId_michlol";
  const isAdmin =
  ...
  const handleFetchData = useCallback(
    async (page, pageSize) => {
      ...
      try {
        ...
        const url = `${baseUrl}/api/${ROUTE}?${queryString}`;
        const res = await fetch(url);
        const { data, rowsCount } = await res.json();
        setRows(data);
        setRowsCount(rowsCount);
      } catch (err) {
        console.error("Failed to fetch michlol data:", err);
      }
  ...
  const handleProcessRowUpdate = useCallback((newRow, oldRow) => {
    ...
    } else if (newRow.shinoa !== oldRow.shinoa && newRow.shinoa !== newRow.manoiya) {
      if (oldRow.shinoa !== newRow.manoiya || manoiyaOptions.includes(newRow.shinoa)) {
        setSnackbar({
          open: true,
  ...
  const handleSaveChanges = useCallback(async () => {
    ...
    try {
      ...
      console.error("Failed to save michlol changes:", err);
      setSnackbar({
        open: true,
  ...
  const handleSubmit = useCallback(
    async (data, isEdit) => {
      try {
        ...
        const res = await fetch(`${baseUrl}/api/${ROUTE}/${data._id}`, {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            updates: data,
  ...
  const handleExportToExcel = useCallback(async () => {
    try {
      ...
      const res = await fetch(
        `${baseUrl}/api/${ROUTE}/export/excel?${params.toString()}`,
      );
  ...
  const fetchEngineCompatibleOptions = useCallback(
    async (sourceId, search, skip, append) => {
      ...
      const res = await fetch(`${baseUrl}/api/repairs/compatible-for-swap?${params.toString()}`);
      if (!res.ok) return;
  ...
  const handleConfirmEngineChange = useCallback(async () => {
    ...
    try {
      const res = await fetch(`${baseUrl}/api/repairs/change-engine-serial`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  const handleConfirmMinseretChange = useCallback(async () => {
    ...
    try {
      const res = await fetch(`${baseUrl}/api/repairs/change-minseret-serial`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  const handleDelete = useCallback(
    async (params) => {
      ...
      if (isAdmin) {
        ...
        try {
          const res = await fetch(`${baseUrl}/api/repairs/${id}`, {
            method: "DELETE",
          });
          if (!res.ok) {
  ...
  }, [visibleColumns, columnsWithActions, isViewer, isAdmin, isManoiya]);
  ...
  return (
    <Box sx={containerSx}>
      <Typography variant="h1" gutterBottom>
        מכלול (חט"כים עם מנוע או ממסרת בלבד)
      </Typography>

      <Box sx={toolbarSx}>
        {user && !isViewer && (
          <>
            {/* No "add" button on Michlol page */}
            <Button
              variant="contained"
              id="saveChanges"
        ...
        <CustomPagination
          ...
          debounceDelay={300}
          resetToPage1={triggerPaginationReset}
          onResetComplete={handlePaginationResetComplete}
        />
      </Box>
